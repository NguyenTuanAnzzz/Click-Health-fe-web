...
const ScanningLayout = ({ 
  ...
  return (
    <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-6 md:p-8 max-w-4xl mx-auto shadow-sm transition-all duration-300">
      ...
      <div className="mb-6 flex items-center justify-between font-inter-tight-small">
        <div>
          ...
          <h2 className="text-2xl font-bold font-inter text-black flex items-center gap-2.5">
            <Icon size={22} className="text-[#2ecea0]" />
            {title}
          </h2>
        </div>
      </div>
      ...
      <div className="relative w-full aspect-video bg-[#f0f1f2] rounded-[20px] overflow-hidden border border-[#244d54]/15 mb-8 flex flex-col items-center justify-center shadow-xs">
        {imageSrc ? (
          ...
          <>
            ...
            <div className="absolute inset-0 pointer-events-none">
              ...
              <div className="absolute top-5 left-5 right-5 flex justify-between items-start font-inter-tight-small">
                <div className="bg-[#244d54]/85 text-white text-[11px] px-3.5 py-1.5 rounded-full flex items-center gap-2.5 backdrop-blur-md font-bold tracking-wider border border-white/10 shadow-md">
                ...
                {isAutoScanning && (
                  <div className="bg-[#2ecea0]/90 text-white text-[10px] px-3 py-1 rounded-full backdrop-blur-md border border-white/10 flex items-center gap-2 font-bold uppercase tracking-wider">
              ...
              {countdown !== null && countdown > 0 && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-3xs">
                  <div className="text-[10rem] font-bold text-[#2ecea0] drop-shadow-2xl animate-pulse font-inter">
                    {countdown}
                  </div>
                </div>
              )}
              ...
              <div className="absolute top-0 left-0 w-10 h-10 border-t-2 border-l-2 border-[#2ecea0] m-6" />
              <div className="absolute top-0 right-0 w-10 h-10 border-t-2 border-r-2 border-[#2ecea0] m-6" />
              <div className="absolute bottom-0 left-0 w-10 h-10 border-b-2 border-l-2 border-[#2ecea0] m-6" />
              <div className="absolute bottom-0 right-0 w-10 h-10 border-b-2 border-r-2 border-[#2ecea0] m-6" />
              
              {/* Circular Reticle Target */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 border border-[#2ecea0]/20 rounded-full animate-pulse" />
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-[#2ecea0] rounded-full shadow-[0_0_8px_#2ecea0]" />
            </div>
          </>
        )}
      </div>
        ...
        {!imageSrc ? (
          <button 
            ...
            className={`flex items-center gap-2.5 px-10 py-4 rounded-full text-[14px] font-bold transition-all duration-300 shadow-md ${
              ...
              : 'bg-[#2ecea0] text-white hover:bg-[#26b38a] hover:scale-[1.01] hover:shadow-[#2ecea0]/15'
            }`}
          >
            {isAutoScanning ? (
              <>
          ...
          <>
            <button 
              onClick={retake}
              disabled={loading}
              className="flex items-center gap-2 bg-white border border-[#e5e7eb] text-[#244d54] px-8 py-3.5 rounded-full text-[14px] font-bold hover:bg-[#f0f1f2] transition-colors duration-300 disabled:opacity-50 shadow-sm"
            >
              <RefreshCw size={16} />
              Quét lại
            </button>
            <button 
              ...
              disabled={loading}
              className="flex items-center gap-2 bg-[#2ecea0] text-white px-10 py-3.5 rounded-full text-[14px] font-bold hover:bg-[#26b38a] hover:scale-[1.01] transition-all duration-300 disabled:opacity-50 shadow-md shadow-[#2ecea0]/15"
            >
              {loading ? (
